...
savePath = 'DataSet/'+project+'/types/'
...
typefilepath = 'DataSet/'+project+"/types/pureJava"
jsonfilepath = 'DataSet/'+project+"/types/rawjson"
...
for javaFile in tqdm(set(typeList)):
    ...
    for i,version in enumerate(versionList):
        ...
        if javaFile in smellVersionDict[version]:
            row.append(1)
        elif javaFile in typeVersionDict[version]:
            row.append(0)
        else:
            row.append(-1)
...
for code in projectVersionSmell:
    ...
    for i in range(len(versionList)):
        ...
        if aa == projectVersionSmell[code] and survival_versions>threshold000 and s_000<s_000_th:
            # 取中间版本
            negPath = aProjectVersionsLabeledPath+versionList[aa.count(-1)+aa.count(0)//2]+'/pureJava/'+code
            negPaths.append(negPath)
            s_000+=1

            # 此类样本不够，则多次采样，如采集首尾数据
            if less1==1:
                negPath = aProjectVersionsLabeledPath+versionList[aa.count(-1)]+'/pureJava/'+code
                negPaths.append(negPath)
                s_000+=1
                negPath = aProjectVersionsLabeledPath+versionList[len(aa)-1]+'/pureJava/'+code
                negPaths.append(negPath)
                s_000+=1
        ...
        if bb == projectVersionSmell[code] and survival_versions>threshold111 and s_111<s_111_th:
            # 取中间样本
            negPath = aProjectVersionsLabeledPath+versionList[bb.count(-1)+bb.count(1)//2]+'/pureJava/'+code
            negPaths.append(negPath)
            s_111+=1
            survival_no_refact_list.append(survival_versions)
            # 此类样本不够，则多次采样，如采集首尾数据
            if less2==1:
                negPath = aProjectVersionsLabeledPath+versionList[bb.count(-1)]+'/pureJava/'+code
                negPaths.append(negPath)
                s_111+=1
                negPath = aProjectVersionsLabeledPath+versionList[len(bb)-1]+'/pureJava/'+code
                negPaths.append(negPath)
                s_111+=1
...
for i,v in enumerate(negPaths):
    #/home/yqx/Downloads/DesigniteJava-master/myData/AntVersionsLabeled/ant-rel-1.9.5/pureJava/org.apache.tools.ant.taskdefs.optional.pvcs-PvcsProject.java
...
for root, ds, files in os.walk(typefilepath):
    for file in tqdm(files):
        codepath = root + '/' + file
        #/home/yqx/Downloads/DesigniteJava-master/myData/Ant/pureJava/ant-rel-1.1-com.oreilly.servlet-MailMessage.java
        #print(codepath)
        #try:
        try:
